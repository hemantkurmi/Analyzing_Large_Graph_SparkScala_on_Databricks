...
val greater = withoutDuplicates.filter($"weight" >= 5)

val srcdf =  greater.toDF("Source","Target","Weight")
                    .select("Source","Weight")
                    .groupBy("Source")
                    .agg(sum("Weight").as("weighted-out-degree"))

val tgtdf = greater.toDF("Source","Target","Weight")
                    .select("Target","Weight")
                    .groupBy("Target")
                    .agg(sum("Weight").as("weighted-in-degree"))

val wdf = srcdf.join(tgtdf, srcdf("Source") === tgtdf("Target") ,"full_outer")
               .withColumn("node", coalesce($"Source", $"Target"))
               .drop("Source").drop("Target")
               .na.fill(0, Seq("weighted-in-degree"))
              .na.fill(0, Seq("weighted-out-degree"))
              
val total = wdf.select($"node", $"weighted-in-degree", $"weighted-out-degree",($"weighted-out-degree" + $"weighted-in-degree").as("weighted-total-degree"))
...
val in = total.orderBy($"weighted-in-degree".desc , $"node".asc).limit(1).select("node","weighted-in-degree").withColumn("c", lit("i"))
val out =total.orderBy($"weighted-out-degree".desc , $"node".asc).limit(1).select("node","weighted-out-degree").withColumn("c", lit("o"))
val ttl = total.orderBy($"weighted-total-degree".desc , $"node".asc).limit(1).select("node","weighted-total-degree").withColumn("c", lit("t"))
...
val  finaldf = in.union(out).union(ttl).toDF("v","d","c");
